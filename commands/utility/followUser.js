...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (userToFollow.bot) {
            await interaction.reply(`Cannot follow a bot.`);
            return;
        }

        if (userToFollow.id == interaction.user.id) {
            await interaction.reply(`Cannot follow yourself.`);
            return;
        }
        ...
        if (currUser && userExists) {
            ...
            if (userFollowList.includes(userExists.user_id)) {
                await interaction.reply(`**${userExists.name}** is already in your follow list.`);
                return;
            }
            
            userFollowList.push(userExists.user_id);
